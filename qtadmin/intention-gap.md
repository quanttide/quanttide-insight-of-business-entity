...
- **三支柱只覆盖一角**：asset 有 archive、status、quality 三个子命令，delib（议事）与 strategy（战略）零实现。但两者都不缺制度素材：议事制度已有完整设计（提案权下放全体、代表审议投票、会前核心小组检查）；战略推演方法论已在洞察库沉淀（`data/insight/strategy`），缺的都是系统侧载体
